...
const TenantChat = () => {
  ...
  const [currentChat, setCurrentChat] = useState(null);
  const [currentSelectedChatIndex, setCurrentChatIndex] = useState(null);
  const { markAsRead, isConnected } = useContext(SocketContext);
  ...
  // REMOVED: Socket message handling from here - let ChatMessages handle it

  const handleCurrentChatChange = (chat, chatId) => {
    // Use provided chatId or determine it from chat
    const finalChatId = chatId || (chat.isNewChat ? chat.chatId : chat._id);

    // Only emit markAsRead for existing chats, not new ones
    if (!chat.isNewChat && isConnected && finalChatId) {
      markAsRead(user?._id, finalChatId);
      dispatch(markChatAsRead({ chatId: finalChatId }));
    }

    setCurrentChat(chat);
    setCurrentChatIndex(finalChatId);
  };
  ...
  // Handle messages for inactive chats (when they're not currently open)
  const handleInactiveChat = (messageData) => {
    console.log("📝 Handling inactive chat message:", messageData);

    // Refresh chats to show updated recent message
    dispatch(getTenantChats());
  };
      ...
      >
        ...
        {currentChat === null ? (
          ...
          <ChatMessages
            ...
            handleCurrentChatChange={handleCurrentChatChange}
            onNewChatCreated={handleNewChatCreated}
            onInactiveChatMessage={handleInactiveChat}
          />
        )}
      </div>
    </div>
